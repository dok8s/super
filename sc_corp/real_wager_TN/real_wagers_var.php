...
if ($rtype=='PL'){
	$mysql="SELECT * FROM `tennis` WHERE `m_Date` ='$mDate' and m_start<now()";
}else{
	$mysql="SELECT * FROM `tennis` WHERE `m_start` > now( ) and `m_Date` ='$mDate'";
}
...
$mysql="SELECT m_league_tw as m_league FROM `tennis` WHERE `m_start` > now( ) and `m_Date` ='$mDate' group by m_league";
$result = mysql_query( $mysql);
...
switch ($rtype){
case "OU":
	$mysql = "select MID,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,m_letb as M_LetB_en,$m_league as M_League,MB_Win,TG_Win,MB_Dime_Rate,TG_Dime_Rate,concat('o',m_dime) as MB_Dime_en,MB_LetB_Rate,TG_LetB_Rate,MB_MID,TG_MID,ShowType,R_Show,s_single,s_double from tennis where`m_start` > now( ) AND `m_Date` ='$mDate'".$sleague."  order by m_start,mid";
	...
	break;
case "PD":
	$mysql = "select MID,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,ShowType,PD_Show from tennis where`m_start` > now( ) AND `m_Date` ='$mDate' order by m_start,mid";
	$result = mysql_query( $mysql);
	$cou=mysql_num_rows($result);
	echo "parent.gamount=$cou;\n";
	...
	break;
case "P":
	$mysql = "select MID,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_Sleague,MB_MID,TG_MID,ShowType from tennis where`m_start` > now( ) AND `m_Date` ='$mDate'".$league." and R_Show=1 order by m_start,mid";
...
case "PL":
	$mDate=date('m-d');
	$mysql = "select * from tennis where `m_Date` ='$mDate' and m_start<now() order by m_start,mid";
	$result = mysql_query( $mysql);
	$cou_num=mysql_num_rows($result);
	$page_size=40;
	$page_count=$cou_num/$page_size;
