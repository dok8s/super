...
switch ($rtype){
case "OU":
	$mysql = "select MID,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,m_letb as M_LetB_en,$m_league as M_League,MB_Win,TG_Win,M_Flat,MB_Dime_Rate,TG_Dime_Rate,concat('o',m_dime) as MB_Dime_en,MB_LetB_Rate,TG_LetB_Rate,MB_MID,TG_MID,ShowType,R_Show,s_single,s_double from volleyball where`m_start` > now( ) AND `m_Date` >'$mDate'".$sleague."  order by m_start,mid";
	$result = mysql_query( $mysql);
	$cou_num=mysql_num_rows($result);
	$page_size=40;
	$page_count=$cou_num/$page_size;
	...
	while ($row=mysql_fetch_array($result)){
		...
		$mid=$row['MID'];
		$sql="select mtype,linetype,count(*) as cou,sum(BetScore) as score FROM  `web_db_io` where MID =".$mid." and  `m_Date` >'$bdate' and LineType<4  group by LineType,Mtype order by LineType,Mtype";
		...
		echo "parent.GameTN[$K] = Array('$row[MID]','$row[M_Date]<br>$row[M_Time]','$league','$row[MB_MID]','$row[TG_MID]','$row[MB_Team]','$row[TG_Team]','$row[ShowType]','$show','$row[M_LetB_en]','$row[M_LetB_en]','$MB_LetB_Rate','$TG_LetB_Rate','$h2c','$c2c','$h2s','$c2s','$row[MB_Dime_en]','$TG_Dime_Rate','$MB_Dime_Rate','$h3c','$c3c','$h3s','$c3s','$row[MB_Win]','$row[TG_Win]','$row[M_Flat]','$h1c','$c1c','$n1c','$h1s','$c1s','$n1s','单','双','0.000','0.000','0','0','0','0','0','0');\n";
		$K=$K+1;
	}
	break;
case "PD":
	$mysql = "select MID,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,ShowType,PD_Show from volleyball where`m_start` > now( ) AND `m_Date` >'$mDate' order by m_start,mid";
	$result = mysql_query( $mysql);
	$cou=mysql_num_rows($result);
	echo "parent.gamount=$cou;\n";
	...
	while ($row=mysql_fetch_array($result)){
		$mid=$row['MID'];
		$sql="select count(*) as cou,sum(BetScore) as score FROM `web_db_io` where MID =".$mid." and  linetype=4 ";
    	$res_data = mysql_query( $sql);
	...
	break;
case "P":
	$mysql = "select MID,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_Sleague,MB_MID,TG_MID,ShowType from volleyball where`m_start` > now( ) AND `m_Date` >'$mDate'".$league." and R_Show=1 order by m_start,mid";
	...
	while ($row=mysql_fetch_array($result)){
		...
		$mid=$row['MID'];
		$sql="select Mid,Mtype,count(*) as cou,sum(betscore) as score FROM `web_db_io` where FIND_IN_SET($mid,MID)>0 AND `m_Date` >'$mDate' and (linetype=7 or linetype=8)  group by Mid,Mtype";
		$res_data = mysql_query( $sql);
...
case "PL":
	$mDate=date('m-d');
	$mysql = "select MID,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,M_LetB,$m_league as M_Sleague,MB_Win,TG_Win,M_Flat,MB_Dime_Rate,TG_Dime_Rate,concat('o',m_dime) as MB_Dime_en,MB_LetB_Rate,TG_LetB_Rate,MB_MID,TG_MID,ShowType,R_Show from volleyball where `m_Date` >'$mDate' and m_start<now() order by m_start,mid";
	$result = mysql_query( $mysql);
	$cou_num=mysql_num_rows($result);
	$page_size=40;
	$page_count=$cou_num/$page_size;
	...
	while ($row=mysql_fetch_array($result)){
		...
		$mid=$row['MID'];
		$sql="select LineType as linetype,Mtype as mtype,count(*) as cou,sum(BetScore) as score FROM  `web_db_io` where FIND_IN_SET($mid,MID)>0 AND `m_Date` >'$bdate'  group by LineType,Mtype";
