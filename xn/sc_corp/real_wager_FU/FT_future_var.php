...
if ($gdate==''){
	$mysql="SELECT m_league_tw as m_league FROM `foot_match` WHERE `m_Date` >'$mDate' group by m_league";
}else{
	$mysql="SELECT m_league_tw as m_league FROM `foot_match` WHERE `m_Date`='$gdate' group by m_league";
}
$result = mysql_query( $mysql);
...
case "PD":
	if ($gdate==''){
		$sql1 = "select MID,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,m_league_tw as league,MB1TG0,MB2TG0,MB2TG1,MB3TG0,MB3TG1,MB3TG2,MB4TG0,MB4TG1,MB4TG2,MB4TG3,MB0TG0,MB1TG1,MB2TG2,MB3TG3,MB4TG4,OVMB,MB0TG1,MB0TG2,MB1TG2,MB0TG3,MB1TG3,MB2TG3,MB0TG4,MB1TG4,MB2TG4,MB3TG4,ShowType,PD_Show from foot_match where mid%2=1 and `m_start` > now( ) AND `m_Date` >'$mDate'".$sleague." order by display,mid limit $offset,40";
	}else{
		$sql1 = "select MID,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,m_league_tw as league,MB1TG0,MB2TG0,MB2TG1,MB3TG0,MB3TG1,MB3TG2,MB4TG0,MB4TG1,MB4TG2,MB4TG3,MB0TG0,MB1TG1,MB2TG2,MB3TG3,MB4TG4,OVMB,MB0TG1,MB0TG2,MB1TG2,MB0TG3,MB1TG3,MB2TG3,MB0TG4,MB1TG4,MB2TG4,MB3TG4,ShowType,PD_Show from foot_match where mid%2=1 and `m_start` > now( ) AND `m_Date` ='$gdate'".$sleague." order by display,mid limit $offset,40";
	}

	$sql2	=	"select mid as mid2,count(*) as cou,sum(BetScore) as score FROM `web_db_io` where linetype=4 and super='$agname' and hidden=0  group by mid order by id";
	$sql="select * from (($sql1) as s) left join (($sql2) as o) on (s.mid=o.mid2)";
	$result = mysql_query( $sql);
	$cou=mysql_num_rows($result);
	echo "parent.gamount=$cou;\n";
...
case "HPD":
	...
	$sql2	=	"select mid as mid2,count(*) as cou,sum(BetScore) as score FROM `web_db_io` where linetype=34 and super='$agname' and hidden=0  group by mid order by id";
	$sql="select * from (($sql1) as s) left join (($sql2) as o) on (s.mid=o.mid2)";
	$result = mysql_query( $sql);
	$cou=mysql_num_rows($result);
	echo "parent.gamount=$cou;\n";
...
case "EO":
	if ($gdate==''){
		$sql1 = "select MID,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,m_league_tw as league,S_Single,S_Double,S_0_1,S_2_3,S_4_6,S_7UP,MB_MID,TG_MID,ShowType,T_Show from foot_match where mid%2=1 and `m_Date` >'$mDate'".$sleague." order by display,mid limit $offset,40";
	}else{
		$sql1 = "select MID,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,m_league_tw as league,S_Single,S_Double,S_0_1,S_2_3,S_4_6,S_7UP,MB_MID,TG_MID,ShowType,T_Show from foot_match where mid%2=1 and  `m_Date`='$gdate'".$sleague." order by display,mid limit $offset,40";
	}
	$sql2	=	"select mid as mid2,sum(if(mtype='ODD',1,0)) as oc,sum(if(mtype='ODD',betscore,0)) as os,	sum(if(mtype='EVEN',1,0)) as ec,sum(if(mtype='EVEN',betscore,0)) as es,sum(if(mtype='0~1',1,0)) as 1c,sum(if(mtype='0~1',betscore,0)) as 1s,	sum(if(mtype='2~3',1,0)) as 2c,sum(if(mtype='2~3',betscore,0)) as 2s,sum(if(mtype='4~6',1,0)) as 4c,sum(if(mtype='4~5',betscore,0)) as 4s,	sum(if(mtype='OVER',1,0)) as 7c,sum(if(mtype='OVER',betscore,0)) as 7s	from web_db_io where linetype in (5,6) and super='$agname'  and hidden=0  group by mid";
	$sql	=	"select * from (($sql1) as s) left join (($sql2) as o) on (s.mid=o.mid2)";
	$result = mysql_query( $sql);
	$cou=mysql_num_rows($result);
	echo "parent.gamount=$cou;\n";
...
case "F":
	if ($gdate=='')	{
		$sql1 = "select MID,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,m_league_tw as league,MBMB+0 as MBMB,MBFT+0 as MBFT,MBTG+0 as MBTG,FTMB+0 as FTMB,FTFT+0 as FTFT,FTTG+0 as FTTG,TGMB+0 as TGMB,TGFT+0 as TGFT,TGTG+0 as TGTG,MB_MID,TG_MID,ShowType from foot_match where mid%2=1 and `m_Date` >'$mDate'".$sleague." order by display,mid limit $offset,40";
	}else{
		$sql1 = "select MID,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,m_league_tw as league,MBMB+0 as MBMB,MBFT+0 as MBFT,MBTG+0 as MBTG,FTMB+0 as FTMB,FTFT+0 as FTFT,FTTG+0 as FTTG,TGMB+0 as TGMB,TGFT+0 as TGFT,TGTG+0 as TGTG,MB_MID,TG_MID,ShowType from foot_match where mid%2=1 and `m_Date`='$gdate'".$sleague." order by display,mid limit $offset,40";
	}
	$sql2	=	"select mid as mid2,count(*) as cou,sum(BetScore) as score FROM `web_db_io` where linetype=14 and super='$agname' and hidden=0  group by mid order by id";
	$sql="select * from (($sql1) as s) left join (($sql2) as o) on (s.mid=o.mid2)";
	$result = mysql_query( $sql);
	$cou=mysql_num_rows($result);
	echo "parent.gamount=$cou;\n";
...
case "P":
	if ($gdate=='')	{
		$sql1 = "select MID,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,m_league_tw as league,MB_MID,TG_MID,ShowType from foot_match where `m_Date`>'$mDate'".$sleague." and mid%2=1 order by display,mid limit $offset,40";
	}else{
		$sql1 = "select MID,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,m_league_tw as league,MB_MID,TG_MID,ShowType from foot_match where `m_Date`='$gdate'".$sleague." and mid%2=1 order by display,mid limit $offset,40";
	}
	...
	sum(if(mtype='N',betscore,0)) as mns from web_db_io where linetype in (7,8) and super='$agname' and hidden=0 group by mid order by id";
	$sql="select * from (($sql1) as s) left join (($sql2) as o) on (s.mid=o.mid2)";
	$result = mysql_query( $sql);
	$cou=mysql_num_rows($result);
	echo "parent.gamount=$cou;\n";
