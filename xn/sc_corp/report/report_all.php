...
if ($row['subuser']==1){
	$agname=$row['subname'];
	$loginfo='子帐号:'.$row['Agname'].'查询期间'.$date_start.'至'.$date_end.$QQ526738.'报表';
}else{
	$agname=$row['Agname'];
...
$sql="select corprator as name,count(*) as coun,sum(betscore) as score,sum(m_result) as result,sum(a_result) as a_result,sum(vgold) as vgold,sum(result_a) as result_a,sum(result_s) as result_s,(100-corpor_point-world_point-agent_point) as point,sum(result_c) as result_c from web_db_io where  ".$where." and super='$agname'";
...
<?
if ($credit=='block'){
	$mysql=$sql." and pay_type=0 group by corprator order by corprator asc";
...
<table border="0" cellpadding="0" cellspacing="1" bgcolor="#000000" class="m_tab"  style="display: <?=$credit?>" width="960">
  ...
  <tr class="m_title_reall" >
	...
	while ($row = mysql_fetch_array($result)){
		...
		$c_vgold_c+=$row['vgold']*$row['point']*0.01;
  	?>
  <tr class="m_rig" onMouseOver="setPointer(this, 0, 'over', '#FFFFFF', '#FFCC66', '#FFCC99');" onMouseOut="setPointer(this, 0, 'out', '#FFFFFF', '#FFCC66', '#FFCC99');">
    ...
    <td><?=number_format($row['result_c'],1)?></td>
    <td><?=number_format($row['vgold'],1)?></td>
    <td><?=number_format($row['vgold']*$row['point']*0.01,1)?></td>
  </tr>
<?
}
?>
 ...
 <tr class="m_rig_to" >
    ...
    <td><?=number_format($c_result_c,1)?></td>
    <td><?=number_format($c_vgold,1)?></td>
    <td><?=number_format($c_vgold_c,1)?></td>
  </tr>
</table>
<!-----------------↑ 信用额度资料区段 ↑------------------------->
<?
...
if ($sgold=='block'){
	$mysql=$sql." and pay_type=1 group by corprator order by corprator asc";
	$result = mysql_query($mysql);
	$cou=mysql_num_rows($result);
	if ($cou==0){
		$sgold='none';
...
<table border="0" cellpadding="0" cellspacing="1" bgcolor="#000000" class="m_tab"  style="display: <?=$sgold?>" width="960">
  ...
  <?
	while ($row = mysql_fetch_array($result)){
		...
		$c_vgold_c1+=$row['vgold']*$row['point']*0.01;
  	?>
  <tr class="m_rig" onMouseOver="setPointer(this, 0, 'over', '#FFFFFF', '#FFCC66', '#FFCC99');" onMouseOut="setPointer(this, 0, 'out', '#FFFFFF', '#FFCC66', '#FFCC99');">
    <td align="center"><?=$row['name']?></td>
    <td><?=$row['coun']?></td>
...
?>
  ...
  <tr class="m_rig_to">
   <td>总计</td>
    ...
    <td><?=number_format($c_result_c1,1)?></td>
    <td><?=number_format($c_vgold1,1)?></td>
    <td><?=number_format($c_vgold_c1,1)?></td>
  </tr>
</table>
<BR>
<?
