...
if ($count==0){
	...
	$tu=rand(1345515,8345915);
     $_SESSION["tktk"]=$tu;
	$loginfo='Đăng nhập người dùng đã thành công';
	$ip_addr = $_SERVER['REMOTE_ADDR'];
	$mysql="insert into web_mem_log(username,logtime,context,logip,level) values('$username',now(),'$loginfo','$ip_addr','0')";
	mysql_query($mysql);

	$sql="update web_super set oid='$uid',logintime=now(),domain='$_SERVER[HTTP_HOST]',language='zh-vn',logip='$ip_addr' where Agname='".$username."'";
	mysql_query($sql) or die ("Thao tác thất bại!");
