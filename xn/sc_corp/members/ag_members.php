...
case 2:
	...
	mysql_query( $mysql);
	$mysql="insert into  agents_log (M_DateTime,M_czz,M_xm,M_user,M_jc,Status) values('".date("Y-m-d H:i:s")."','$agname','$xm','$memname','Thành viên',2)";
...
case 3:
	...
	//mysql_query( $mysql);
	$mysql="insert into  agents_log (M_DateTime,M_czz,M_xm,M_user,M_jc,Status) values('".date("Y-m-d H:i:s")."','$agname','Xóa','$agent_name','Thành viên',2)";
	mysql_query($mysql) or die ("Thao tác thất bại!");
	break;
case 8:
	$mysql="update web_member set oid='',Status=$stop where ID=$mid";
...
function so()
{
	...
	if(so_username==''){
		alert('Vui lòng nhập tài khoản thành viên hoặc một phần của tài khoản thành viên để tìm kiếm');
	}else{
  ...
  <tr>
	<td class="">
        <table border="0" cellspacing="0" cellpadding="0" >
          <tr>
            <td width="60">Quản lý thành viên</td>
            <td>
		...
		?>
			...
			<td>Tài khoản (hoặc một phần)：<INPUT TYPE="text" size=10 name="so_username" id="so_username" value='<?=$_GET['so_username']?>'> <INPUT TYPE="button" VALUE="Tìm kiếm" ONCLICK="so();"></td>
          </tr>
        </table>
	</td>
</tr>
...
<?
	...
	?> <tr class="m_cen">
      ...
      <td align="left"><font color="#0000FF"><a style="cursor: hand">
		...
		if($enable=='Y'){
		?>
			<a href="javascript:CheckSTOP('./ag_members.php?uid=<?=$uid?>&active=8&id=<?=$row['ID']?>&enable=S','S')">Tạm ngưng</a> /
		<?
		}
		?>
<a href="javascript:CheckSTOP('./ag_members.php?uid=<?=$uid?>&active=2&id=<?=$row['ID']?>&enable=<?=$memstop?>','<?=$memstop?>')"><?=$caption1?></a>
/ <a href="./ag_mem_edit.php?uid=<?=$uid?>&mid=<?=$row['ID']?>">Sửa đổi dữ liệu</a>
/ <a href="ag_mem_set.php?uid=<?=$uid?>&pay_type=0&mid=<?=$row['ID']?>&aid=<?=$row['Agents']?>">Cài đặt chi tiết</a>
